#!/usr/bin/env python3
"""
Simple test for the refactored tokenizer without unicode characters.
"""
...
def test_simple_command():
    ...
    print("Actual tokens:")
    for token in tokens:
        print(f"  - text='{token.text}', position={token.position}, type={token.token_type}")
    
    assert len(tokens) == 3
    assert tokens[0].text == "create" and tokens[0].position == 0
    assert tokens[1].text == "company" and tokens[1].position == 7
    assert tokens[2].text == "ACME" and tokens[2].position == 15
    print("PASSED")
...
if __name__ == "__main__":
    try:
        ...
        test_mixed_syntax()
        test_edge_cases()
        print("\nALL TESTS PASSED! Tokenizer refactoring is working correctly.")
