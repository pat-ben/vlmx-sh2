...
from ..models.schema.base import DatabaseSchema, DatabaseModel
from ..models.context import ContextLevel


def generate_entity_words(schema: Type[DatabaseSchema]) -> Dict[str, EntityWord]:
    """
    ...
    for entity_cls in schema.tables:
        if not issubclass(entity_cls, DatabaseModel):
            continue
...
def generate_field_words(schema: Type[DatabaseSchema]) -> Dict[str, FieldWord]:
    """
    ...
    field_to_entities: Dict[str, List[Type[DatabaseModel]]] = defaultdict(list)
    ...
    for entity_cls in schema.tables:
        ...
        for field_name, field_info in entity_cls.model_fields.items():
            ...
            if field_name not in field_to_descriptions:
                ...
                field_to_descriptions[field_name] = field_description or f"Field '{field_name}'"
...
def generate_schema_words(schema: Type[DatabaseSchema]) -> Dict[str, Word]:
    """
