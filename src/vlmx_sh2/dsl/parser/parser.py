...
Important: This module is NOT a parsing stage itself - it orchestrates stages 0-6 only.
...
from .filter import parse as filter_parse
...
def parse(input_text: str, context: Context) -> TokensResult:
    """
    ...
    pipeline_result = _run_pipeline(input_text, validation_context, context)
    if pipeline_result is None:
        # Pipeline failed early - return empty result
        return _build_tokens_result(
            input_text, [], [], None, validation_context
        )
...
def _run_pipeline(
    input_text: str, validation_context: ValidationContext, context: Context
) -> Optional[tuple]:
    """
    ...
    tokens = tokenize(normalized_text, validation_context)

    if validation_context.has_errors():
        return None  # Stop on tokenization errors

    # Stage 2: Classifier (BLOCKING)
    classified_tokens = classify(tokens, validation_context)
    ...
    recognized_tokens = recognize(classified_tokens, validation_context)
    ...
    interpreted_tokens = interpret(recognized_tokens, context)

    # Continue even with interpretation errors

    # Stage 5: Splitter (NON-BLOCKING)
    split_result = split(interpreted_tokens, validation_context)
    ...
    filter_expression = filter_parse(split_result, validation_context)
