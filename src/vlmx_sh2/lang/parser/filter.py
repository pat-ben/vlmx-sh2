...
class Filter:
    ...
    def _add_error(self, message: str, token_text: str = "") -> None:
        ...
        self.context.add_error(
            stage=IssueStage.FILTER, message=message, token_text=token_text
        )
    ...
    def _parse_or_expression(self) -> Optional[FilterExpression]:
        ...
        while self._current_token() and self._is_or_keyword(self._current_token()):
            ...
            left_expr = FilterExpression(
                left=left_expr, operator=LogicalOperator.OR, right=right_expr
            )
        ...
        while self._current_token():
            ...
            if self._is_and_keyword(current):
                ...
                if self._has_error or right_expr is None:
                    return None
                left_expr = FilterExpression(
                    left=left_expr, operator=LogicalOperator.AND, right=right_expr
                )
    ...
    def _parse_grouped_expression(self) -> Optional[FilterExpression]:
        ...
        if not close_paren or not self._is_close_paren(close_paren):
            if close_paren:
                self._add_error(
                    f"Expected ')' but found '{close_paren.text}'", close_paren.text
                )
            else:
    ...
    def _parse_simple_condition(self) -> Optional[FilterExpression]:
        ...
        if available < 3:
            self._add_error(
                f"Condition requires field, operator, and value (found {available} tokens)"
            )
            return None
        ...
        if (
            not operator_token
            or not hasattr(operator_token, "operator")
            or not operator_token.operator
        ):
    ...
    def _parse_value_or(self) -> Optional[ValueExpression]:
        ...
        while (
            self._current_token()
            and self._is_or_keyword(self._current_token())
            and self._is_in_value_context()
        ):
            self._consume_token()  # consume 'OR' / '|'
            ...
            left_expr = ValueExpression(
                left=left_expr, logic=LogicalOperator.OR, right=right_expr
            )
    ...
    def _parse_value_and(self) -> Optional[ValueExpression]:
        ...
        while (
            self._current_token()
            and self._is_and_keyword(self._current_token())
            and self._is_in_value_context()
        ):
            self._consume_token()  # consume 'AND' / '&'
            ...
            if self._has_error or right_expr is None:
                return None
            left_expr = ValueExpression(
                left=left_expr, logic=LogicalOperator.AND, right=right_expr
            )
    ...
    def _parse_range_value(self, start_value: str) -> Optional[ValueExpression]:
        ...
        if (
            next_token
            and self._is_value_token(next_token)
            and not self._is_next_condition_start()
        ):
            # Consume end value
    ...
    def _is_value_token(self, token: Optional[InterpretedToken]) -> bool:
        ...
        if not token:
            return False
        return not (
            token.operator or token.query_word or token.range_word or token.bracket
        )
