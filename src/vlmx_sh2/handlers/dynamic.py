...
from ..ui.results import CommandResult, create_error_result, create_success_result
from .utils import (
...
async def add_dynamic_handler(
    parse_result: ParseResult, context: Context
) -> CommandResult:
    """
    ...
    try:
        ...
        for attr_name in attributes.keys():
            if not validate_attribute_for_entity(attr_name, entity_name):
                validation_errors.append(
                    f"Attribute '{attr_name}' is not valid for entity '{entity_name}'"
                )
        ...
        if not save_result.get("success", False):
            return create_error_result(
                [save_result.get("error", f"Failed to save {entity_name} data")]
            )

        # Create success message
        attr_list = [f"{name}={value}" for name, value in attributes.items()]
        return create_success_result(
            operation="added", entity_name=entity_name, attributes=attributes
        )
...
async def update_dynamic_handler(
    parse_result: ParseResult, context: Context
) -> CommandResult:
    """
    ...
    try:
        ...
        if not entity_exists(entity_name, company_name, context):
            return create_error_result(
                [f"Entity '{entity_name}' does not exist for company '{company_name}'"]
            )
        ...
        attr_list = [f"{name}={value}" for name, value in attributes.items()]
        return create_success_result(
            operation="updated", entity_name=entity_name, attributes=attributes
        )
...
async def show_dynamic_handler(
    parse_result: ParseResult, context: Context
) -> CommandResult:
    """
    ...
    try:
        ...
        return create_success_result(operation="displayed", entity_name=entity_name)
...
async def delete_dynamic_handler(
    parse_result: ParseResult, context: Context
) -> CommandResult:
    """
    ...
    try:
        ...
        if not specific_attributes:
            return create_error_result(
                ["No attributes specified. Use format: delete entity attribute"]
            )
        ...
        if not removed_attributes:
            return create_error_result(
                [f"None of the specified attributes exist in {entity_name}"]
            )
        ...
        return create_success_result(operation="removed", entity_name=entity_name)
