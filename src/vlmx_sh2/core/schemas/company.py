...
from vlmx_sh2.core.enums import Legal, Currency, Unit, TypeOrg, Country, Stage, Phase, Sector, Model, Round, NewsCategory, CompetitorSize, Cardinality
...
class OrganizationEntity(EntityModel, table=True):
    ...
    currency: Optional[Currency] = Field(default=Currency.USD, description="Operating currency")
    ...
    incorporation: Optional[date] = Field(default=None, description="Date of incorporation")
...
class AddressEntity(EntityModel, table=True):
    ...
    headquarter: bool = Field(default=True, description="Is this the headquarter of the company?")
...
class NewsEntity(EntityModel, table=True):
    ...
    # ==================== USER FIELDS ====================
    news_date: date = Field(default_factory=date.today, description="Date of the news article")
...
class CompetitorsEntity(EntityModel, table=True):
    ...
    similarity: Optional[float] = Field(default=0.0, description="Similarity score between the two companies")
    ...
    leader: Optional[bool] = Field(default=False, description="Is the competitor a leader in the industry")
