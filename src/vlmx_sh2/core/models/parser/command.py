...
class ParsedCommand(BaseModel):
    ...
    field_values: Dict[str, str] = Field(
        default_factory=dict, description="Field-value pairs for entity attributes"
    )
    ...
    filter_tokens: List[RecognizedToken] = Field(
        default_factory=list, description="Recognized tokens from filter expression"
    )

    # Filtering
    filters: Optional[FilterExpression] = Field(
        default=None, description="Parsed filter expression for list/show commands"
    )
