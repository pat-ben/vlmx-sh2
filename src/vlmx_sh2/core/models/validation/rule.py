...
class ValidationRule(BaseModel):
    ...
    check: Callable[..., bool] = Field(
        ..., description="Validation function - returns True if valid"
    )
    error_code: str = Field(
        ...,
        description="Structured error code (e.g., 'vlmx::tokenizer::empty_command')",
    )
    message: Union[str, Callable[..., str]] = Field(
        ..., description="Human-readable error message or function that generates one"
    )
    suggestion: Union[str, Callable[..., str]] = Field(
        default="",
        description="Optional suggestion for fixing or function that generates one",
    )
