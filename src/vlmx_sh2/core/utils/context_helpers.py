...
def validate_command_requirements(
    context: Context, has_schema: bool = False, has_app: bool = False
    ...
    if len(missing) == 1:
        if missing[0] == "schema":
            return (
                False,
                f"At {get_level_name(context).upper()} level, you must specify the schema: command [company_name] ...",
            )
        else:  # app
            return (
                False,
                f"At {get_level_name(context).upper()} level, you must specify the app: command ... [app_name]",
            )
    else:  # both missing
        return (
            False,
            f"At {get_level_name(context).upper()} level, you must specify both schema and app: command [company_name] ... [app_name]",
        )
...
def command_requires_schema(
    action_word, context: Context, entity_value: Optional[str] = None
) -> bool:
    """
...
def validate_command_context_requirements(
    action_word,
    context: Context,
    ...
    if len(missing) == 1:
        if missing[0] == "schema":
            if action_word.id == "cd":
                return (
                    False,
                    f"At {level_name} level, you must specify the company: cd [company_name]",
                )
            else:
                return (
                    False,
                    f"At {level_name} level, you must specify the schema: {command_name} [company_name] ...",
                )
        else:  # app
            return (
                False,
                f"At {level_name} level, you must specify the app: {command_name} ... [app_name]",
            )
    else:  # both missing
        return (
            False,
            f"At {level_name} level, you must specify both schema and app: {command_name} [company_name] ... [app_name]",
        )
