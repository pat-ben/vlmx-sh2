...
from ..models.responses import ColumnSpec, FieldSpec

FieldType = Literal["text", "number", "date", "select", "boolean", "textarea"]
...
def build_field_specs(
    ...
    for field_name in field_names:
        ...
        field_type: FieldType = _infer_field_type(field_info)
        ...
        field_spec = FieldSpec(
            ...
            options=_extract_field_options(field_info)
...
def _get_field_default(field_info) -> Optional[Any]:
    ...
    return (
        getattr(field_info, "default", None) if hasattr(field_info, "default") else None
    )


def _generate_placeholder(field_name: str, field_type: FieldType) -> Optional[str]:
